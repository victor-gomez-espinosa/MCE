...
class MissingValueImputation():
    ...
    def __init__(self, method = 'list_wise', k = 10, learning_rate=0.3, n_estimators=150, subsample=0.9, gamma=10, reg_lambda=10,neur_Hid_layer=[50,50],epoch=100):
        self.method = method
    ...
    def __k_nearest_neighbors_imputation(self, df):
        ...
        pipe_reg = Pipeline([('scaler', StandardScaler()), ('knn', KNeighborsRegressor(n_neighbors=self.k))]) #pipe regresion
        pipe_clas = Pipeline([('scaler', StandardScaler()), ('knn', KNeighborsClassifier(n_neighbors=self.k))]) #pipe class
        ...
        for column in columns: #recorre las columnas con datos faltantes
            ...
            if col_type=='float64' or col_type=='int64': #si es de tipo numérico
                y=pipe_reg.fit(X_train, y_train).predict(X) #regresion
            else: #categorico
                y=pipe_clas.fit(X_train, y_train).predict(X) #clasificacion
    ...
    def __trees_imputation(self, df):
        ...
        for column in columns: #recorre las columnas con datos faltantes
            ...
            if col_type=='float64' or col_type=='int64': #si es de tipo numérico
                rnd=xgb.XGBRegressor(learning_rate=self.learning_rate,n_estimators=self.n_estimators,subsample=self.subsample,gamma=self.gamma,reg_lambda=self.reg_lambda,n_jobs=-1)
                
                rnd.fit(X_train,y_train,eval_set=[(X_train, y_train)],eval_metric='mae',early_stopping_rounds=2)
                y=rnd.predict(X_pred) #regresion
            else: #categorico
                if num_classes ==2:
                    rnd=xgb.XGBClassifier(learning_rate=self.learning_rate,n_estimators=self.n_estimators,subsample=self.subsample,gamma=self.gamma,reg_lambda=self.reg_lambda,n_jobs=-1)
                    
                    rnd.fit(X_train,y_train,eval_set=[(X_train, y_train)],eval_metric='logloss',early_stopping_rounds=2)
                    y=rnd.predict(X_pred) #clasificacion
                else:
                    rnd=xgb.XGBClassifier(objective='multi:softmax',learning_rate=self.learning_rate,n_estimators=self.n_estimators,subsample=self.subsample,gamma=self.gamma,reg_lambda=self.reg_lambda,n_jobs=-1)
                    
                    rnd.fit(X_train,y_train,eval_set=[(X_train, y_train)],eval_metric='mlogloss',early_stopping_rounds=2)
                    y=rnd.predict(X_pred) #clasificacion
